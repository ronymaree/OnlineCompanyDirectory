...
$sql = "SELECT * FROM combination_table Where company_id = $company_id";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        $company_services[$row['service_id']][] = $row['cat_id']; 
        ...
        if (mysqli_num_rows($result_request) > 0) {
        	while($row_request = mysqli_fetch_assoc($result_request)) {
        		...
        		print '<tr onClick = "readValues(this)">';
        			...
        			print '<td><a href="#popup" data-company="'.$company_id.'"  data-request="'.$row_request['request_id'].'" class="up">Send Quote</a></div></td>';
